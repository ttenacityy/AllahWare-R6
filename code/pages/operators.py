...
class OperatorsPage(ctk.CTkFrame):
    ...
    def _load_all_operators(self):
        for side in ["attackers", "defenders"]:
            ...
            for name in self.all_operators[side]:
                if name in self.operator_images:
    ...
    def _select_operator(self, name):
        ...
        control_page = self.app.pages.get("Control")
        if control_page:
            control_page.y_slider.set(cfg["Y"])
            control_page.x_slider.set(cfg["X"])
            control_page.y_value.set(f"{cfg['Y']:.3f}")
            control_page.x_value.set(f"{cfg['X']:.3f}")
